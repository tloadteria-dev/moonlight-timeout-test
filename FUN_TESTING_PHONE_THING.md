...
The one-click runner has a read-only USB/irecovery query path (`irecovery -q`). It has no physical-device authentication-mutation path and cannot change Face ID state on an iPhone.
